...
;$.fn.eventCalendar = function (options) {
    ...
    function dateSlider(show, year, month) {
        var $eventsCalendarSlider = $("<div class='eventsCalendar-slider'></div>"),
            ...
            $eventsCalendarArrows = $("<a href='#' class='arrow prev'><span class='icon-arrow-left'>" + eventsOpts.txt_prev + "</span></a><a href='#' class='arrow next'><span class='icon-arrow-right'>" + eventsOpts.txt_next + "</span></a>");
        $eventsCalendarDaysList = $("<ul class='eventsCalendar-daysList'></ul>"),
            date = eventsOpts.monthToShow;
        ...
        if (show === "current") {
            ...
            if (date) {
                $calendar.trigger('monthUpdate', (date));
            }
        ...
        var data_date;
        var empty_days = daysList.length - 7;
            ...
            if (data.length) {
                ...
                $.each(data, function (key, event) {
                    ...
                    if (eventYear == flags.wrap.attr('data-current-year') && eventMonth == flags.wrap.attr('data-current-month')) {
                        var day_li = flags.wrap.find('.currentMonth .eventsCalendar-daysList #dayList_' + parseInt(eventDay)).addClass('dayWithEvents'),
                            day_a = day_li.find('.show-event-btn');
                        day_a.attr('data-type', event.type).attr('data-id', event.id).addClass(event.type);

                        var element = flags.wrap.find('.currentMonth .eventsCalendar-daysList #dayList_' + parseInt(eventDay));
                        element.addClass('dayWithEvents');
                        if (eventsOpts.showTooltip) {
                            var child = flags.wrap.find('.currentMonth .eventsCalendar-daysList #dayList_' + parseInt(eventDay) + ' a');
                            child.addClass('event-day-tooltip');
                            var child_id = '#day-event-' + parseInt(eventDay);
                            child.attr('title', eventsOpts.tooltipText);
                            $(child).tooltip();
                        }
                    }
